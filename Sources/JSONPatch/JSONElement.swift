...
extension JSONElement {
    ...
    public mutating func apply(_ operation: JSONPatch.Operation, options: [JSONPatch.ApplyOption] = []) throws {
        ...
        } catch JSONError.referencesNonexistentValue where options.contains(.ignoreNonexistentValues) {
...
extension JSONElement {
    ...
    func get(index: Int) -> JSONElement? {
        guard self.isContainer, let dict = self.rawValue as? NSDictionary else { return nil }
        
        return try? JSONElement(any: dict.allValues[index])
    }
}
